...
if ($nv_Request->isset_request('delete_subject_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($subject_id > 0 and $delete_checkss == md5($subject_id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Subjects', 'ID: ' . $subject_id, $admin_info['userid']);
...
$row = array();
$error = array();
$row['subject_id'] = $nv_Request->get_int('subject_id', 'post,get', 0);
if ($nv_Request->isset_request('submit', 'post')) {
    $row['subject_name'] = $nv_Request->get_title('subject_name', 'post', '');
    ...
    if (empty($error)) {
        try {
            if (empty($row['subject_id'])) {
                $stmt = $db->prepare('INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_subjects (subject_name, status, add_time, update_time) VALUES (:subject_name, :status, :add_time, :update_time)');
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_subjects SET subject_name = :subject_name, status = :status, add_time = :add_time, update_time = :update_time WHERE subject_id=' . $row['subject_id']);
            }
            $stmt->bindParam(':subject_name', $row['subject_name'], PDO::PARAM_STR);
            ...
            if ($exc) {
                $nv_Cache->delMod($module_name);
                if (empty($row['subject_id'])) {
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Add Subjects', ' ', $admin_info['userid']);
                } else {
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit Subjects', 'ID: ' . $row['subject_id'], $admin_info['userid']);
                }
                nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
            }
...
} elseif ($row['subject_id'] > 0) {
    $row = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_subjects WHERE subject_id=' . $row['subject_id'])->fetch();
    if (empty($row)) {
        nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op);
    }
} else {
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    if (!empty($q)) {
        $db->where('subject_name LIKE :q_subject_name OR status LIKE :q_status OR add_time LIKE :q_add_time OR update_time LIKE :q_update_time');
    }
    $sth = $db->prepare($db->sql());

    if (!empty($q)) {
        $sth->bindValue(':q_subject_name', '%' . $q . '%');
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;subject_id=' . $view['subject_id'];
        $view['link_delete'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;delete_subject_id=' . $view['subject_id'] . '&amp;delete_checkss=' . md5($view['subject_id'] . NV_CACHE_PREFIX . $client_info['session_id']);
